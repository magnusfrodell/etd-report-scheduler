...
from app.reports.analysis import recipients_of, technique_types, url_host
from app.reports.base import ReportContext
from app.settings_store import THREAT_VERDICTS

FAMILIES: dict[str, tuple[str, ...]] = {
    "Impersonation": ("brand impersonation", "domain brand impersonation", "sender domain brand impersonation",
    ...
    "Social engineering": ("call to action", "urgency", "data input request", "request for credentials",
    ...
    "Malicious link": ("malicious url", "link masquerade", "open redirect", "shortened url", "reused url",
                       "victim specific url", "low-reputation tld", "qr code"),
    "Malicious attachment": ("malicious html attachment", "masqueraded file extension"),
    "Evasion": ("hidden text", "hidden text injection", "image-only email", "email without text"),
    "Sender reputation": ("disposable sender address", "rare sender address", "rare sender domain",
    ...
    "Known relationship": ("frequent sender", "frequent sender for recipient", "frequent sender for recipient's domain",
                           "internal email"),
}
FAMILY_NOTES = {
    "Impersonation": "Pretends to be a brand, colleague or the victim. Maintain High Impact Personnel and brand lists in ETD.",
    "Social engineering": "Pressure and requests - credentials, contact details, opening attachments.",
    "Malicious link": "Link-borne attacks incl. redirects, shorteners and QR codes.",
    "Malicious attachment": "HTML smuggling and disguised file types.",
    "Evasion": "Hidden text and image-only mail built to fool filters.",
    "Sender reputation": "New, rare or disposable senders and poor infrastructure reputation.",
    "Known relationship": "Threats from senders the recipient normally talks to - often a compromised partner account.",
}
_FAMILY_OF = {name: fam for fam, names in FAMILIES.items() for name in names}

ATTACHMENT_CLASSES: tuple[tuple[str, frozenset[str]], ...] = (
    ("HTML / SVG", frozenset({"html", "htm", "shtml", "xhtml", "svg", "svgz", "mht", "mhtml"})),
    ("Archive / disk image", frozenset({"zip", "rar", "7z", "gz", "tgz", "tar", "ace", "arj", "cab", "iso", "img", "vhd", "vhdx", "z"})),
    ("Script / shortcut", frozenset({"js", "jse", "vbs", "vbe", "wsf", "wsh", "hta", "lnk", "bat", "cmd", "ps1", "psm1", "url", "reg", "jar", "scf", "inf"})),
    ("Executable / installer", frozenset({"exe", "dll", "msi", "msix", "appx", "appxbundle", "com", "pif", "scr", "cpl", "xll"})),
    ("Office with macros / legacy", frozenset({"docm", "dotm", "xlsm", "xltm", "xlam", "pptm", "potm", "ppam", "doc", "xls", "ppt", "rtf"})),
    ("Office", frozenset({"docx", "xlsx", "pptx", "odt", "ods", "odp"})),
    ("PDF", frozenset({"pdf"})),
    ("OneNote", frozenset({"one", "onepkg"})),
    ("Calendar invite", frozenset({"ics", "vcs"})),
    ("Image", frozenset({"png", "jpg", "jpeg", "gif", "bmp", "webp", "tif", "tiff", "heic"})),
)
...
ABUSED_SERVICES: dict[str, tuple[str, ...]] = {
    ...
    "Microsoft Forms / Sway / Dynamics": ("forms.office.com", "forms.microsoft.com", "sway.cloud.microsoft", "sway.office.com", "dynamics.com"),
    "Azure hosting": ("blob.core.windows.net", "web.core.windows.net", "azurewebsites.net", "azureedge.net", "azurefd.net"),
    "Google services": ("docs.google.com", "drive.google.com", "sites.google.com", "forms.gle", "storage.googleapis.com", "firebaseapp.com", "web.app", "script.google.com"),
    "Cloudflare": ("pages.dev", "workers.dev", "r2.dev", "trycloudflare.com"),
    "Developer hosting": ("github.io", "githubusercontent.com", "netlify.app", "vercel.app", "glitch.me", "replit.app", "onrender.com", "herokuapp.com", "fly.dev", "surge.sh"),
    "Site builders": ("webflow.io", "wixsite.com", "weebly.com", "squarespace.com", "wordpress.com", "blogspot.com", "godaddysites.com", "framer.app", "carrd.co", "notion.site", "canva.site", "yolasite.com"),
    "File sharing": ("dropbox.com", "dropboxusercontent.com", "wetransfer.com", "we.tl", "box.com", "mega.nz", "mediafire.com"),
    "Forms / surveys": ("typeform.com", "jotform.com", "surveymonkey.com", "formstack.com", "hsforms.com", "paperform.co"),
    "Tunnels / IPFS": ("ngrok.io", "ngrok-free.app", "ngrok.app", "loca.lt", "ipfs.io", "dweb.link", "cloudflare-ipfs.com", "w3s.link"),
    "URL shorteners": ("bit.ly", "t.co", "tinyurl.com", "is.gd", "cutt.ly", "rebrand.ly", "ow.ly", "t.ly", "shorturl.at", "rb.gy", "s.id", "buff.ly", "goo.gl"),
}
...
def family_of(technique: str) -> str:
    return _FAMILY_OF.get(technique.strip().lower(), "Other")
...
def payload_kind(msg: ConvictedMessage) -> str:
    ...
    if link and att:
        return "Link and attachment"
    if link:
        return "Link only"
    if att:
        return "Attachment only"
    return "No link or attachment"
...
def build(session: Session, ctx: ReportContext) -> dict[str, Any]:
    ...
    technique_rows = [
        ...
        for n, c in tech.most_common(30)
    ]
    family_rows = [
    ...
    risk_rows = [{"risk": r, "messages": c, "previous": prev_risk.get(r, 0), "share": _pct(c, total)} for r, c in risk.most_common()]

    payload = Counter(payload_kind(m) for m in msgs)
    prev_payload = Counter(payload_kind(m) for m in prev)
    payload_rows = [{"kind": k, "messages": payload.get(k, 0), "previous": prev_payload.get(k, 0), "share": _pct(payload.get(k, 0), total)}
                    for k in ("Link only", "Attachment only", "Link and attachment", "No link or attachment")]
    ...
    attachment_rows = [{"class": c, "count": n, "risky": c in RISKY_CLASSES} for c, n in att_class.most_common()]
    ...
    service_rows = [{"service": s, "urls": n, "hosts": [h for h, _ in service_hosts[s].most_common(3)]} for s, n in services.most_common()]
    known_relationship = fam.get("Known relationship", 0)

    recs: list[str] = []
    risky_att = sum(att_class[c] for c in RISKY_CLASSES)
    if att_class.get("HTML / SVG"):
        recs.append(f"Block or strip .html/.htm/.svg attachments at the gateway - {att_class['HTML / SVG']} HTML/SVG attachment(s) carried threats (HTML smuggling, SVG phishing).")
    if att_class.get("Script / shortcut") or att_class.get("Executable / installer"):
        recs.append("Block script, shortcut and executable attachments outright; they have no business use in mail.")
    if att_class.get("Archive / disk image"):
        recs.append("Inspect or quarantine archives and disk images (.zip/.iso/.img) - they are used to slip executables past filters.")
    if double or masqueraded:
        recs.append(f"{len(double) + masqueraded} attachment(s) disguised their real type (double extension or masqueraded extension) - show file extensions on endpoints.")
    if qr:
        recs.append(f"{len(qr)} threat(s) used QR codes, which move the click to unmanaged phones - include quishing in awareness training.")
    if callback:
        recs.append(f"{len(callback)} text-only lure(s) without a link look like callback phishing - teach finance and the service desk to verify phone numbers independently.")
    if bec_nopayload:
        recs.append(f"{len(bec_nopayload)} BEC message(s) carried no payload at all - payment-change requests need out-of-band verification.")
    if known_relationship:
        recs.append(f"{known_relationship} threat(s) came from senders ETD recognises as regular correspondents - see the Vendor risk report for possibly compromised partners.")
    if fam.get("Impersonation"):
        recs.append(f"Impersonation in {fam['Impersonation']} threat(s) - keep High Impact Personnel and brand lists up to date in ETD.")
    if service_rows:
        top = service_rows[0]
        recs.append(f"Most-abused legitimate service: {top['service']} ({top['urls']} URL(s)) - consider stricter handling or sandboxing of links to it.")
    ...
    return {
        ...
        "top_extensions": att_ext.most_common(10),
