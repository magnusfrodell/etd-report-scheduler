...
def _conviction_gap(tenant: Any, period: Any, stats_threats: int, convicted: int) -> str | None:
    ...
    if collected is None:
        return "Convicted messages have not been collected yet, so remediation cannot be assessed."
    if collected < period.end:
        return f"Convicted messages were last collected {collected:%Y-%m-%d %H:%M} UTC, before the period ended."
    if stats_threats and not convicted:
        return f"The daily statistics count {stats_threats} threat(s), but no convicted messages were collected for the period."
    return None
...
def build(session: Session, ctx: ReportContext) -> dict[str, Any]:
    ...
    conviction_gap = _conviction_gap(tenant, p, cur.threats, len(threats))
    if conviction_gap:
        checks.append(check("Threats remediated", "unknown", 20, conviction_gap))
        checks.append(check("Remediation is automatic", "unknown", 10, conviction_gap))
    elif threats:
        cov = len(remediated) / len(threats) * 100
        checks.append(check("Threats remediated", "ok" if cov >= 99 else ("warning" if cov >= 90 else "critical"), 20,
                            f"{len(remediated)} of {len(threats)} threats were moved or deleted ({cov:.1f} %).",
                            "Some threat verdicts have no action - check for monitor-only policies and remediation errors in ETD."))
        auto = len(automatic) / len(remediated) * 100 if remediated else 0.0
        checks.append(check("Remediation is automatic", "ok" if auto >= 90 else ("warning" if auto >= 70 else "critical"), 10,
                            f"{auto:.0f} % of remediations were automatic.",
                            "Enable automatic remediation for every threat verdict so nothing waits for an analyst."))
    else:
        checks.append(check("Threats remediated", "na", 20, "No threats in the period."))
        checks.append(check("Remediation is automatic", "na", 10, "No threats in the period."))
    ...
    if conviction_gap:
        checks.append(check("Dwell time for retro verdicts", "unknown", 10, conviction_gap))
    elif exposure:
        median_exposure = percentile(exposure, 0.5)
        still_open = f", {len(open_retro)} still not remediated" if open_retro else ""
        checks.append(check("Dwell time for retro verdicts",
                            "ok" if median_exposure <= 1 and not open_retro else ("warning" if median_exposure <= 4 else "critical"), 10,
                            f"Median {fmt_hours(median_exposure)} from delivery to remediation for {len(exposure)} retro-convicted message(s){still_open}.",
                            "Retro-convicted mail sits in inboxes too long - make sure retrospective verdicts trigger automatic remediation."))
    else:
        checks.append(check("Dwell time for retro verdicts", "na", 10, "No retrospective verdicts in the period."))
    if conviction_gap:
        checks.append(check("No threats through allow-lists", "unknown", 10, conviction_gap))
    elif not threats:
        checks.append(check("No threats through allow-lists", "na", 10, "No threats in the period."))
    elif rule_data:
        checks.append(check("No threats through allow-lists", "ok" if not allow else ("warning" if len(allow) <= 3 else "critical"), 10,
                            f"{len(allow)} threat(s) matched an allow/safe rule.",
                            "Review allow-list rules: threats were delivered because a rule trusted the sender."))
    else:
        checks.append(check("No threats through allow-lists", "unknown", 10, "The API returned no rule information for this period."))
    if own_checks:
        ...
        checks.append(check("DMARC enforcement on own domains", status, 20,
                            ", ".join(f"{c['domain']}: {c['dmarc']['status']}" for c in own_checks),
                            "Move every own domain to DMARC p=reject - spoofing your own domain is the cheapest attack there is."))
        spf_levels = [c["spf"]["level"] for c in own_checks]
        checks.append(check("SPF on own domains", "ok" if all(x == "ok" for x in spf_levels) else ("critical" if "critical" in spf_levels else "warning"), 5,
                            ", ".join(f"{c['domain']}: {c['spf']['status']}" for c in own_checks),
                            "Publish one SPF record per domain ending in -all or ~all, within 10 DNS lookups."))
        with_tls = sum(1 for c in own_checks if c["mta_sts"] and c["tls_rpt"])
        checks.append(check("MTA-STS and TLS-RPT", "ok" if with_tls == len(own_checks) else ("warning" if with_tls else "critical"), 5,
                            f"{with_tls} of {len(own_checks)} domain(s) publish both.",
                            "Publish MTA-STS and TLS-RPT so inbound TLS cannot be downgraded silently."))
    else:
        checks.append(check("DMARC enforcement on own domains", "unknown", 20, "No own domains known or DNS unavailable."))
    cov = audit["coverage"]
    if not settings.log_export_enabled or cov["status"] in ("no_data", "disabled"):
        checks.append(check("Audit log collected", "critical", 10, "Log Export is not collected, so there is no audit trail beyond ETD's 30 days.",
                            "Enable audit and message logs in ETD (Administration > Business > Export Log Preferences) and in Settings."))
    else:
        partial = cov.get("partial_files", 0)
        checks.append(check("Audit log collected", "ok" if cov["pct"] >= 95 and not partial else ("warning" if cov["pct"] >= 50 else "critical"), 10,
                            f"{cov['pct']} % of the period's hours collected, {len(cov['gaps'])} gap(s)"
                            + (f", {partial} file(s) with unreadable lines" if partial else "") + ".",
                            "Keep the collector running - ETD deletes audit logs after 30 days, so gaps cannot be filled later."))
    days_pct = cur.days_with_data / p.days * 100 if p.days else 0
    checks.append(check("Data collection healthy", "ok" if days_pct >= 95 and not tenant.last_error else ("warning" if days_pct >= 70 else "critical"), 5,
                        f"Statistics for {cur.days_with_data} of {p.days} days" + (f"; last error: {tenant.last_error}" if tenant.last_error else "") + ".",
                        "Fix collector errors on the Tenants page so reports cover the whole period."))
    checks.append(check("VIPs defined", "ok" if vips else "warning", 5, f"{len(vips)} VIP mailbox(es) configured.",
                        "List executives, finance and IT admins as VIPs so attacks on them are flagged."))
    if audit["fp_rate"] is not None and (audit["to_neutral"] or audit["to_threat"] or audit["reclass_by_method"]):
        fp = audit["fp_rate"]
        checks.append(check("False-positive rate", "ok" if fp <= 2 else ("warning" if fp <= 5 else "critical"), 5,
                            f"{audit['to_neutral']} manual reclassification(s) to neutral against {audit['threats']} threats ({fp} %).",
                            "Review what analysts release as neutral and tune policies or allow rules accordingly."))
    else:
        checks.append(check("False-positive rate", "unknown", 5, "No verdict-change data (Log Export) for the period."))
    ...
    for start, end in _months(p.end_day):
        ...
        trend.append({"month": start.strftime("%b %Y"), "total": t.total_messages, "threats": t.threats,
